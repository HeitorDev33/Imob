...
caminho_csv = "residencial.csv" 

# 2. Carregar os dados
try:
    df = pd.read_csv(caminho_csv,sep=';', encoding='utf8')
    
    # 3. Preparar o array para a análise
    precos_array = df['total_transações'].values
    ...
    if medidas_calculadas:
        gerar_painel_boxplot(
            precos_array, 
            medidas_calculadas, 
            titulo_boxplot='Boxplot de Total de Transações (residencial.csv)', 
